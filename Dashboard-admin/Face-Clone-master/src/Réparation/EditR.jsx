...
export default function Editboutique() {
 ...
 const navigate =useNavigate()
  useEffect(()=>{
    axios.get(`http://localhost:4000/reparationcompany/getcontratbyidrep/${idr}`)
...
.catch (err=>console.log(err))
 ...
 const handleSubmit = (event) => {
    ...
    axios.put(`http://localhost:4000/reparationcompany/updaterepcompany/${idr}`, data)
      .then(response => {
        Swal.fire("Enregistré !", "", "success");
       navigate('/Réparation')
    })
      .catch(error => {
        console.error('Échec de la mise à jour de l"entreprise de réparation ', error);
    ...
    <div className='a1'>
      ...
      <div id='m5'>  <a href='/Réparation' id='a5'> <div className='m4' id='a4'><ArrowBackIcon/> </div> </a></div>  
        <div className="heading">Modifier Entreprise de réparation</div>
        <form className="form " onSubmit={handleSubmit} >
        <input  className="inputeditboutique scale-in-hor-left " type="text" name="idr"  value={data.idr} onChange={e => setData({...data, idr: e.target.value})}  placeholder='Entrer le nouveau ID'/>
        ...
        <select className="inputeditboutique scale-in-hor-left" name="categorie" value={data.localisation} onChange={e => setData({...data, localisation: e.target.value})}>
        <select className="input" name="categorie" value={data.categorie} onChange={e => setData({...data, categorie: e.target.value})} required>
